...
// Ejefald på dansk: navne, der ender på s, x eller z, får apostrof (SAS' fly), ikke -s
const gen = (s) => String(s) + (/[sxzSXZ]$/.test(String(s)) ? "'" : 's');
...
const FAQ_GENERAL = [
  ...
  { q: 'Ændrer reglerne sig i 2027?', a: 'Ja, men ikke beløbene eller grænsen på 3 timer. EU vedtog i juli 2026 nye regler, der gælder fra 2027: en frist på 9 måneder for at kræve kompensation, besked fra selskabet inden 96 timer, svar inden 30 dage og ret til selv at købe ny rejse for op til 400 % af billetprisen, hvis selskabet ikke ombooker inden 3 timer. <a href="/regler/nye-regler-2027/">Læs om ændringerne</a>.' },
  { q: 'Hvad hvis flyselskabet siger, det var vejret?', a: 'Vejr, der reelt umuliggør flyvningen, er som regel en usædvanlig omstændighed. Men selskabet skal både dokumentere vejret ved netop din flyvning og vise, at det ikke kunne have undgået forsinkelsen, fx ved at ombooke dig hurtigere. Bed om begge dele skriftligt. Du har under alle omstændigheder ret til forplejning, ombookning eller refusion.' },
];
const FAQ_RULES = [
...
const FAQ_2027 = [
  { q: 'Får jeg mindre i kompensation fra 2027?', a: 'Nej. Beløbene 250, 400 og 600 € og grænsen på 3 timer er uændrede. På korte og mellemlange ruter kan selskabet ikke længere halvere beløbet ved ombookning, så nogle får mere.' },
  { q: 'Gælder de nye regler for min rejse i 2026?', a: 'Nej. De nye regler gælder først for flyvninger fra anvendelsesdatoen, som ligger 12 måneder efter offentliggørelsen i EU-Tidende, altså i 2027. Rejser før da følger de nuværende regler, herunder den danske forældelse på 3 år.' },
  { q: 'Kan jeg stadig søge 3 år tilbage?', a: 'For flyvninger før anvendelsesdatoen, ja. For flyvninger fra 2027 skal kravet sendes til selskabet inden 9 måneder efter afgang. Har du gamle krav, så søg dem nu.' },
  { q: 'Hvad sker der, hvis selskabet ikke svarer inden 30 dage?', a: 'Fra 2027 er det et brud på forordningen, som du kan klage over til Trafikstyrelsen. Selskabet skal enten betale eller begrunde afvisningen med henvisning til det konkrete punkt på listen over usædvanlige omstændigheder.' },
  { q: 'Er strejke stadig selskabets ansvar?', a: 'Ja. Strejke blandt selskabets eget personale er kun en usædvanlig omstændighed, hvis kravene alene kan opfyldes af offentlige myndigheder. Almindelige løn- og arbejdstidsstrejker giver stadig kompensation. Strejke hos flyveledere og lufthavne er stadig som regel undtaget.' },
  { q: 'Skal jeg betale for håndbagage fra 2027?', a: 'Én personlig genstand skal altid være gratis. Selskabet må stadig tage betaling for en større håndbagage, men prisen med håndbagage skal vises som standard, før du begynder at booke.' },
  { q: 'Hvad er de 400 %?', a: 'Har du valgt ombookning, og selskabet ikke har tilbudt en inden 3 timer, må du selv købe en ny rejse. Selskabet skal refundere dine nødvendige og rimelige udgifter op til 400 % af den oprindelige billetpris inden 14 dage.' },
];
const FAQ_SELF = [
  ...
  { q: 'Kan jeg klage på vegne af hele familien?', a: 'Ja, hvis I rejste på samme booking. Skriv alle passagerer i samme e-mail. Børn med egen billet har krav på fuldt beløb. Spædbørn uden eget sæde har ikke krav på kompensation.' },
...
const STATIC = [
  ...
  ['nye-regler-2027', '/regler/nye-regler-2027/', 'Nye regler for flypassagerer fra 2027: det ændrer sig | Tjek dit fly', 'EU har vedtaget nye regler for flypassagerer. Beløbene og grænsen på 3 timer er uændrede. Nyt: frist på 9 måneder, besked fra selskabet inden 96 timer, svar inden 30 dage og ombookning med 400 %-loft. Gælder fra 2027.', { faq: FAQ_2027, breadcrumb: [['Forside', '/'], ['Reglerne', '/regler/'], ['Nye regler fra 2027', '/regler/nye-regler-2027/']] }],
  ...
  ['sammenlign', '/sammenlign/', 'Sammenlign 8 selskaber, der henter flykompensation | Tjek dit fly', 'Otte selskaber, der henter flykompensation mod salær. Sammenlignet på salær, salær ved retssag og det, du får udbetalt. Gebyrer tjekket ' + providersFile.checked + '.', { faq: FAQ_COMPARE }],
  ...
  ['annoncelinks', '/annoncelinks/', 'Sådan tjener Tjek dit fly penge: annoncelinks, og hvad de betyder', 'Tjek dit fly skal finansieres af annoncelinks. Her kan du læse, hvordan det foregår, hvilke aftaler der findes lige nu, og hvad det betyder for indholdet.'],
...
for (const [file, p, title, description, opts = {}] of STATIC) {
  ...
  await emit({ path: p, title, description, body, faq: opts.faq, breadcrumb: opts.breadcrumb, dateModified: gitDate(`src/pages/${file}.html`) });
}
...
<p><strong>Fra 2027</strong> forsvinder trinnet på 7 til 13 dage: kompensationen bortfalder kun ved besked mindst 14 dage før, eller ved en ombookning, der afgår højst 1 time før og ankommer under 2 timer efter planen. <a href="/regler/nye-regler-2027/">Se alle ændringerne</a>.</p><p>Beskeden skal være givet til dig. Sendte selskabet e-mailen til rejsebureauet, som ikke gav den videre, hæfter selskabet stadig (Krijgsman-sagen, 2017). Det er selskabet, der skal bevise, hvornår du fik besked.</p>
...
<h2>Dine rettigheder</h2><ul><li>Kompensation på 250, 400 eller 600 € <strong>med det samme</strong>, i lufthavnen eller senest inden 7 dage.</li><li>Valget mellem refusion og ombookning til din destination.</li><li>Forplejning og hotel, hvis ombookningen først er næste dag.</li><li>Placeres du i en lavere klasse, skal 30 % (op til 1.500 km), 50 % (1.500 til 3.500 km og alle ruter inden for EU) eller 75 % (længere ruter uden for EU) af billetprisen refunderes oveni.</li></ul>
...
<tr><td>Forsinket bagage</td><td>21 dage fra den dag, du fik den</td><td>Rimelige udgifter til tøj, toiletsager og andet nødvendigt i ventetiden. Gem kvitteringer. Køb ikke nyt til hele ferien.</td></tr>
...
<h2>Bagage er ikke en sag for kompensationsselskaberne</h2><p>Flyhjælp og de andre kompensationsselskaber tager ikke bagagesager; der er ingen fast kompensation at tage procent af. Du skal selv skrive til selskabet. Brug selskabets bagageformular, henvis til Montreal-konventionens artikel 17 og 19, og vedhæft PIR-rapport, kvitteringer og bagagemærke.</p>`,
    faq: [
      { q: 'Min kuffert kom 3 dage for sent på ferien. Hvad kan jeg kræve?', a: 'Rimelige udgifter til det nødvendige i de 3 dage: tøj, undertøj, toiletsager, evt. badetøj. Gem kvitteringerne og send dem til selskabet inden 21 dage efter, at du fik kufferten. Nogle selskaber betaler et fast dagsbeløb i stedet.' },
      ...
      { q: 'Kufferten var ødelagt, men jeg opdagede det først hjemme. Er det for sent?', a: 'Fristen er 7 dage fra modtagelsen for skader. Er du inden for 7 dage, så skriv straks til selskabet med billeder og bagagemærke. Er fristen overskredet, er kravet mod selskabet som regel tabt, men rejseforsikringen kan stadig dække.' },
    ] },
};
...
for (const [key, s] of Object.entries(SITUATIONS)) {
  const cta = key === 'bagage' ? '' : key === 'strejke' ? ctaBox({ h: 'Var det selskabets egen strejke?', p: `Så har du et krav som ved enhver anden aflysning, og du kan lade Flyhjælp tage sagen mod ${FH_FEE} af beløbet, som de kun får, hvis de vinder. Var det flyveledere eller lufthavnen, der strejkede, er der som regel ingen kompensation at hente, og du skal i stedet sikre dig refusion eller ombookning direkte hos selskabet.` }) : ctaBox();
...
for (const al of airlines) {
  ...
  const faq = [
    ...
    { q: `Hvordan klager jeg til ${al.name}?`, a: `Brug den formular til kompensationskrav, selskabet har på sin hjemmeside, eller send vores <a href="/klag-selv/">skabelon</a> til kundeservice. Angiv bookingnummer, flynummer, dato og din endelige ankomsttid. Får du ikke svar inden 6 uger, kan du klage til Trafikstyrelsen, hvis flyet afgik fra Danmark.` },
...
<section class="faq"><h2>Ofte stillede spørgsmål om ${esc(al.name)}</h2>${faqHtml(faq)}</section>` });
  ...
  for (const k of CLAIM_SITUATIONS) {
    ...
    const faq2 = [
      ...
      ...(k === 'strejke' ? [{ q: `Strejker ${gen(al.name)} eget personale, eller er det andre?`, a: `Strejker piloter eller kabinepersonale hos ${al.name}, er det selskabets egen strejke, og du har krav på kompensation. Strejker flyveledere eller lufthavnen, har du ikke. Spørg ${al.name}, hvem der strejkede, og bed om det på skrift.` }] : []),
    ...
    await emit({ path: `/flyselskab/${al.slug}/${k}/`, title: `${title} | Tjek dit fly`, description: `${s.name} med ${al.name}: dine rettigheder, beløbet og vejen til at klage. ${al.zone === 'eu' ? 'Både ud- og hjemrejser er dækket.' : 'Kun afgange fra EU er dækket.'}`,
...
<h2>Sådan klager du til ${esc(al.name)}</h2><p>${esc(al.note)}</p><p>Skriv til <a href="${esc(al.claim)}" rel="noopener" target="_blank">kundeservice hos ${esc(al.name)}</a> med bookingnummer, flynummer, dato og faktisk ankomsttid, og henvis til forordning 261/2004 artikel 7. Brug vores <a href="/klag-selv/">skabelon</a>. Får du ikke svar inden 6 uger, kan du klage gratis til Trafikstyrelsen, hvis flyet afgik fra Danmark.</p></section>
${k === 'strejke' ? ctaBox({ h: `Var det ${gen(al.name)} egen strejke?`, p: `Så har du et krav som ved enhver anden aflysning, og Flyhjælp kan tage sagen mod ${FH_FEE} af beløbet, som de kun får, hvis de vinder. Var det flyveledere eller lufthavnen, der strejkede, er der som regel ingen kompensation at hente.` }) : ctaBox({ h: `Lad Flyhjælp tage sagen mod ${al.name}`, p: `Flyhjælp fører sager om ${s.short} mod ${al.name} og de andre selskaber, der flyver fra Danmark. De tager ${FH_FEE} af kompensationen inkl. moms, og kun, hvis du får pengene.` })}
...
- De nye regler fra 2027 (vedtaget juli 2026): ${SITE}/regler/nye-regler-2027/

...
import { haversineKm, compensationFor, coverage, assess, assistanceThresholdH } from '/assets/eu261.js?v=__RULES_V__';
...
export async function initLetter() {
  ...
  function write() {
    ...
    const earlyH = hvad === 'aflyst' ? Number(v('tidligere') || 0) : 0;
    const earlyDa = earlyH ? (earlyH >= 3 ? ' Erstatningsflyet afgik desuden mere end 2 timer tidligere end planlagt.' : ' Erstatningsflyet afgik desuden op til ' + earlyH + ' time' + (earlyH === 1 ? '' : 'r') + ' tidligere end planlagt.') : '';
    const earlyEn = earlyH ? (earlyH >= 3 ? ' The replacement flight also departed more than 2 hours earlier than scheduled.' : ' The replacement flight also departed up to ' + earlyH + ' hour' + (earlyH === 1 ? '' : 's') + ' earlier than scheduled.') : '';
